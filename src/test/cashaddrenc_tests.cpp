...
public:
    ...
    static bool IsScriptDst(const CTxBCHDestination &d) {
        DstTypeChecker checker;
        boost::apply_visitor(checker, d);
        return checker.isScript;
    }

    static bool IsKeyDst(const CTxBCHDestination &d) {
        DstTypeChecker checker;
        boost::apply_visitor(checker, d);
...
BOOST_AUTO_TEST_CASE(encode_decode) {
    std::vector<CTxBCHDestination> toTest = {CNoDestination{},
    ...
    for (auto dst : toTest) {
        for (auto net : GetNetworks()) {
            ...
            CTxBCHDestination decoded = DecodeCashAddr(encoded, *netParams);
...
BOOST_AUTO_TEST_CASE(invalid_on_wrong_network) {
    const CTxBCHDestination dst = CKeyID(uint160S("c0ffee"));
    const CTxBCHDestination invalidDst = CNoDestination{};
    ...
    for (auto net : GetNetworks()) {
        for (auto otherNet : GetNetworks()) {
            ...
            CTxBCHDestination decoded = DecodeCashAddr(encoded, *otherNetParams);
...
BOOST_AUTO_TEST_CASE(random_dst) {
    ...
    for (size_t i = 0; i < NUM_TESTS; ++i) {
        ...
        const CTxBCHDestination dst_key = CKeyID(hash);
        const CTxBCHDestination dst_scr = CScriptID(hash);

        const std::string encoded_key = EncodeCashAddr(dst_key, *params);
        const CTxBCHDestination decoded_key = DecodeCashAddr(encoded_key, *params);

        const std::string encoded_scr = EncodeCashAddr(dst_scr, *params);
        const CTxBCHDestination decoded_scr = DecodeCashAddr(encoded_scr, *params);
...
BOOST_AUTO_TEST_CASE(check_padding) {
    ...
    const CTxBCHDestination nodst = CNoDestination{};
    const auto params = CreateChainParams(CBaseChainParams::MAIN);

    for (uint8_t i = 0; i < 32; i++) {
        data[data.size() - 1] = i;
        std::string fake = cashaddr::Encode(params->CashAddrPrefix(), data);
        CTxBCHDestination dst = DecodeCashAddr(fake, *params);
...
BOOST_AUTO_TEST_CASE(test_encode_address) {
    ...
    for (size_t i = 0; i < hash.size(); ++i) {
        const CTxBCHDestination dstKey = CKeyID(uint160(hash[i]));
        ...
        const CTxBCHDestination dstScript = CScriptID(uint160(hash[i]));
