...
std::vector<uint8_t> PackAddrData(const T &id, uint8_t type) {
    ...
    ConvertBCHBits<8, 5, true>(converted, std::begin(data), std::end(data));

    return converted;
}

// Implements encoding of CTxBCHDestination using cashaddr.
...
std::string EncodeCashAddr(const CTxBCHDestination &dst,
                           const CChainParams &params) {
...
CTxBCHDestination DecodeCashAddr(const std::string &addr,
                              const CChainParams &params) {
    std::cout << "cashaddrenc decodecashaddr 0\n";
    CashAddrContent content =
        DecodeCashAddrContent(addr, params.CashAddrPrefix());
    if (content.hash.size() == 0) {
    std::cout << "cashaddrenc decodecashaddr 1\n";
        return CNoDestination{};
    }

    std::cout << "cashaddrenc decodecashaddr 2\n";
    std::cout << "cashaddrenc string: " << addr << std::endl;
...
CashAddrContent DecodeCashAddrContent(const std::string &addr,
    ...
    ConvertBCHBits<5, 8, false>(data, begin(payload), end(payload));
...
CTxBCHDestination DecodeCashAddrDestination(const CashAddrContent &content) {
